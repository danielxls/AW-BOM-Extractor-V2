...
export const extractBOM = async (file: File, onProgress: (message: string) => void): Promise<BOMRecord[]> => {
  ...
  try {
    ...
    onProgress("Sending document to Gemini AI for analysis...");
    ...
    throw new Error(`Failed to extract data from ${file.name}. The model may have had trouble parsing the file.`);
  }
};

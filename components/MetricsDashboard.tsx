...
const MetricCard: React.FC<{ title: string; value: string | number; }> = ({ title, value }) => (
  <div className="bg-white dark:bg-brand-gray-900 p-6 rounded-xl shadow-sm">
    <h3 className="text-sm font-medium text-brand-gray-500 dark:text-brand-gray-400">{title}</h3>
    <p className="text-3xl font-bold text-brand-gray-900 dark:text-brand-gray-100 mt-2">{value}</p>
  </div>
);
...
const MetricsDashboard: React.FC<MetricsDashboardProps> = ({ data }) => {
  ...
  const metrics = useMemo(() => {
    ...
    // FIX: By casting the initial value of `reduce` to `Record<string, number>`,
    // we ensure `supplierCounts` is correctly typed. This fixes the type inference
    // for `Object.entries`, which in turn resolves the arithmetic error in the `sort` function.
    ...
    const topSupplier = Object.keys(supplierCounts).length > 0
      // FIX: Explicitly type the sort callback parameters to solve the arithmetic operation error.
      ? Object.entries(supplierCounts).sort((a: [string, number], b: [string, number]) => b[1] - a[1])[0][0]
      : 'N/A';
  ...
  if (data.length === 0) {
    return (
      <div className="text-center p-8 bg-white dark:bg-brand-gray-900 rounded-xl shadow-sm h-full flex flex-col justify-center items-center">
        <Icon name="upload" className="w-16 h-16 text-brand-gray-300 dark:text-brand-gray-700 mx-auto mb-4" />
        <h3 className="text-xl font-semibold text-brand-gray-800 dark:text-brand-gray-200">No Data for Dashboard</h3>
  ...
  return (
    <div className="p-4 sm:p-8 h-full">
        <h1 className="text-3xl font-bold text-brand-gray-900 dark:text-brand-gray-100 mb-6">Current Session Dashboard</h1>
        ...
        <div className="mt-8 bg-white dark:bg-brand-gray-900 p-6 rounded-xl shadow-sm">
             <h2 className="text-xl font-bold text-brand-gray-900 dark:text-brand-gray-100 mb-4">Session Details</h2>
             <p className="text-brand-gray-600 dark:text-brand-gray-400">This dashboard reflects statistics only for the documents processed in the current session. Historical data and trend analysis would be available in a full production environment.</p>
        </div>
    </div>
  );
};
